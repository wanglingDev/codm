...
public class FloatService extends Service {
    ...
    @Override public int onStartCommand(Intent i, int flags, int id) {
        startFg();
        // Inject first, then launch CODM, then show overlay
    ...
    // ════════════════════════════════════════════════════════════════════════
    //  INJECT → LAUNCH → SHOW OVERLAY (sequential)
    // ════════════════════════════════════════════════════════════════════════
    private void injectThenLaunch() {
        // 1) Check if game already running
        String pid = MemPatcher.getPid();

        if (pid == null) {
            // Launch CODM first, then wait for it
            launchCODM();
        ...
        if (pid == null) {
            showToast("CODM not found after 60s");
            showOverlay();
            return;
        }
        ...
        showToast(anyOk ? "Injected! Switch to CODM" : "Inject failed — check root");
    ...
    private View buildMenuView() {
        ...
        statusTv.setText(injected
            ...
            : "✖  Not injected");
        statusTv.setTextColor(injected ? 0xFF4CAF50 : 0xFFFF3B3B);
